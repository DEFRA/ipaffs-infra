...
latestBuildId="$(az pipelines runs list --org https://dev.azure.com/defragovuk/ --project DEFRA-EUTD-IPAFFS --query-order FinishTimeDesc --result succeeded --status completed --top 1 --pipeline-ids "${pipelineId}" --branch "${BRANCH}" --query '[0].id')"
