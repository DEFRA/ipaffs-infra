...
readonly DOCUMENT1_XPATH6='/flow-definition/definition/scm/userRemoteConfigs/hudson.plugins.git.UserRemoteConfig/url'
readonly DOCUMENT1_XPATH7='/flow-definition/definition/scm/userRemoteConfigs/hudson.plugins.git.UserRemoteConfig/credentialsId'
  ...
  case "${docment_xpaths##*/}" in
    remote|url)
      if ! [[ "${elementValue}" == "${DEFRA_GITHUB_PATH_PREFIX}"* ]]; then
        ...
        updatedElementValue="${elementValue}"
      fi
      ;;
    credentialsId)
...
if [[ -n "${filename}" ]]; then
  if ! ((dry_run)); then
    ...
    cp "${filename}" "${filename}-$(date +%Y%m%d)-${random}"
    echo -e ":: Created backup: ${filename}-$(date +%Y%m%d)-${random}"
